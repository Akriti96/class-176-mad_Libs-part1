...
$(function(){
    $(".input_field").keyup(function(){
        ...
        var inputboxNumber= id.split("_")[1]
        console.log(inputboxNumber)

        $(".rep_input").eq(inputboxNumber).html($(this).val())
        // $('#input_fields').empty()
...
function displaystrory() {
    randomNumber = parseInt(Math.random() * stories.length)
    console.log(randomNumber)

    const mystory = stories[randomNumber]
    console.log(mystory)

    //    title
    $('#story_title').html(mystory.title)

    // content
    $("#story_text").html(mystory.story)

    // inputboxes
    $('#input_fields').empty()
    for (var i=0;i<mystory.inputs;i+=1){
        var input_html=`<input type="text" class="input_field" id="input_${i}" placeholder="Input ${i+1}"/>`
        $('#input_fields').append(input_html)
    }

    $("#bank_words").empty()
    for(var i=0; i<mystory.words.length;i+=1){
        var button= `<button class="word_bank_button">${mystory.words[i]}</button>`
        $("#bank_words").append(button)
    }
